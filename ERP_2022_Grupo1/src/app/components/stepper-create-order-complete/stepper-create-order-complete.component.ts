...
export class StepperCreateOrderCompleteComponent implements OnInit {
  ...
  constructor(public SupplierService: SupplierService, public ProductService: ProductService, private OrderService : OrderService, private OrderLinesService: OrderLinesService,private dialog:MatDialog) {}
  ...
  onChangeObj(selectedid:number) {    
    ...
    console.log(this.idsupplierSeleccionado)
  ...
  aniadirProducto(productoSeleccionado: ClsProductLine){
    ...
    console.log(this.arrayCart.length)
      for ( i = 0, i < this.arrayCart.length && !salir; i++;) {     
        ...
        this.total == Number(this.total) + Number(producto.unitPrice) * Number(producto.amount);       
      }      
      if(unico)
